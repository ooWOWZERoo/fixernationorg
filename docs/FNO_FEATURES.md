...
- **Cron jobs** — scheduled jobs for Morning Boost email sends (direct sender — see the recurring-campaigns note above for its in-progress replacement, with a reversible kill switch via `Setting` key `morning_boost_direct_send_enabled`), scheduled campaign sending and recurring-campaign dispatch (both share one underlying send implementation, `sendCampaignNow` in `src/lib/send-campaign.ts`, with stuck-send recovery), automation ticking, application expiration + reminder emails, account-invite reminders, and expired-token cleanup. Each job uses database locking so overlapping runs can't double-fire; recurring dispatch additionally claims each day's slot atomically per template via a unique constraint, immune to that race even under overlapping invocations.
