...
- **Campaigns (`/admin/campaigns`, `/new`, `/[id]`, `/[id]/edit`)** — 7-step creation wizard (details → content → UTM/tracking → test send → audience → schedule → review); A/B variants with traffic split; delivery/open/click/bounce metrics. The Content step's subject-line field has an "insert Morning Boost title" picker beneath it (reuses the same content-lookup endpoint the visual body composer's Morning Boost card uses) — picking an entry appends its title to the subject text. The list page groups campaigns by health rather than creation date — Needs attention (stuck in Sending past 30 minutes, or a Scheduled send that's overdue and hasn't started — both states the daily campaign-scheduler/campaign-recovery cron jobs only catch once a day) / Sending now / Scheduled / Sent / Draft, with Paused & cancelled collapsed at the bottom — and its stat strip includes avg bounce rate alongside avg open rate, also shown per-campaign in the table. Bounded to the last 90 days (recurring templates always included) since a daily recurring campaign alone adds 365+ rows/year.
- **Recurring campaigns** — the Schedule step's "Recurring" option turns a campaign into a perpetual template: every day it clones itself into a fresh child occurrence (its own tracking, own audience snapshot) and sends that. Currently ships as Daily, email-only, no A/B — the schema supports Weekly and other content sources for later, but Vercel's Hobby-plan cron limit (once per day, no finer granularity) means every recurring campaign fires at the same fixed 7:00 AM UTC slot rather than an admin-chosen time; a future Pro-tier upgrade could reactivate true per-template times. One content source exists today: "Today's Morning Boost," which calls the same email builder real Morning Boost subscribers already get (full parity — title, author, image, body, CTA) and skips a day entirely rather than repeating yesterday's content if nothing new was published, or if the same entry would otherwise fire twice. Templates get their own section on the campaigns list (pause/resume, occurrence history) instead of sitting in "Draft" forever; each template's detail page has a "Preview next send" dry-run button.
...
- **Automation Journeys (`/admin/automations`, `/[id]`)** — visual step-builder (trigger: manual/signup/role change/tag added/application accepted/group join/event RSVP/loyalty milestone; steps: wait, send email, add/remove tag, webhook, send push, condition-branch, exit); built-in starter templates. The list page (`/admin/automations`) leads with a stat strip (active journeys, running enrollments, completions this week, journeys needing attention) and groups journeys by activity — Needs attention (any outstanding failed enrollment) / Active-running / Active-idle / Inactive (collapsed by default) — with a colored status dot per row, instead of one flat table sorted by creation date. Each journey's own detail page carries the same idea: a per-status count strip (active/completed/failed/paused/cancelled) replaces the old single lifetime enrollment count, and the Enrollments tab has status filter chips — filtering by Failed queries that status directly so a failed enrollment can't silently fall off the page once 50 newer ones pile up.
...
- **Cron jobs** — scheduled campaign sending and recurring-campaign dispatch, including Morning Boost, now fully unified on one send implementation (`sendCampaignNow` in `src/lib/send-campaign.ts`, with stuck-send recovery); plus automation ticking, application expiration + reminder emails, account-invite reminders, and expired-token cleanup. Each job uses database locking so overlapping runs can't double-fire; recurring dispatch additionally claims each day's slot atomically per template via a unique constraint, immune to that race even under overlapping invocations. (The legacy direct Morning Boost sender that predated the recurring-campaign system has been removed — it fired on its own hardcoded UTC schedule with zero dedup against the new system, which caused a real duplicate/wrong-time send incident.)
...
- **Email system** — all transactional and marketing email goes through cPanel SMTP (no third-party ESP). Templates support `{{variable}}` substitution; campaign emails get an automatic unsubscribe footer and open-tracking pixel. Every failed send (from any caller — password reset, verify-email, Morning Boost, campaigns, admin notifications) is recorded centrally to an `EmailFailure` table, and the admin dashboard (`/admin`) shows a red warning banner if any sends have failed in the last 24 hours, with the most recent error and recipient — added after a hosting-level mailbox suspension went unnoticed because failures were only ever silently logged to the server console.
...
- **Active as of 2026-09-02 — outgoing email is suspended at the hosting level.** The `noreply@fixernation.org` mailbox is rejecting all outgoing mail (`550 Outgoing mail from "noreply@fixernation.org" has been suspended.`) — confirmed via a live send attempt, not a code issue. This breaks password resets, verification emails, Morning Boost, and campaigns until the mailbox is reinstated in cPanel/with the hosting provider. The admin dashboard now shows a warning banner for this (see §8), which is how any future recurrence will surface.
