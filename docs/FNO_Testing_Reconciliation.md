...
7. ~~Ask the Fixer, contact form~~ — **done** (`tests/e2e/public-contact-forms.spec.ts`, 2026-08-22). Reality check first: the original guideline described Ask The Fixer as member-only with allowance limits, a usage counter, expired-member blocking, and an emergency/PHI warning — **none of that is built.** `/ask-the-fixer` and its API (`src/pages/api/ask-the-fixer.ts`) are fully public and unauthenticated: anyone, signed in or not, can submit and it's stored straight to `FixerQuestion` with an admin email notification. No membership check, no rate-limit-by-role, no PHI disclaimer anywhere on the page. That's a genuine spec-vs-reality gap, not a bug — flagging it rather than building the gating/allowance feature, which is out of scope for a testing pass. The contact form (`/contact`, `src/pages/api/contact.ts`) matches its guideline reasonably well: all listed required fields exist, honeypot + 5/hour-per-IP rate limiting were already implemented server-side, retention is implicit (rows just persist, no TTL — consistent with how every other submission model in this app works). No CRM/`Contact` linkage for either form — messages land in their own `ContactMessage`/`FixerQuestion` tables with no connection to the CRM contact model used elsewhere in admin; flagging as a possible future enhancement, not a bug, since there's no spec requirement tying them together. **Found and fixed a real bug**: both forms' APIs already checked for an `_hp` honeypot field, but neither form's JSX rendered the corresponding hidden input — the honeypot only worked against a bot that already knew the exact API contract and POSTed directly, not against the more common case of a headless-browser bot that fills every field it finds in the actual rendered form. Added the hidden input to both (`src/pages/contact.tsx`, `src/pages/ask-the-fixer.tsx`). Tests cover: client-side required-field blocking (no network call fires), a full valid submission with DB verification, server-side zod validation catching a too-short message/body that slips past the browser's `required`-only (no `minlength`) check, the honeypot now actually working, and one dedicated test that runs the shared 5-per-hour-per-IP rate limiter to its 6th, rejected request. **Infra note:** both forms' endpoints share a `RateLimitEntry` table keyed by IP — since these tests hit real production endpoints and this suite runs isolated-then-full-parallel per deploy, added a `resetRateLimit()` DB helper (deletes matching `RateLimitEntry` rows directly) called in `beforeAll`/`afterAll` rather than relying on 5 requests/hour being enough headroom across repeated runs. Also hit the same write/read lag documented elsewhere in this suite (automation enrollments, loyalty points) between the app's write and a `TEST_DATABASE_URL` read — both valid-submission checks poll with a 40s window instead of asserting immediately. Separately, Ask The Fixer's API awaits an SMTP admin-notification send before responding at all (unlike the contact form, which sends no email) — widened that one UI assertion's timeout to 20s so a normal SMTP round-trip doesn't read as a stuck submit button.
