...
const GAME_KEY = "WELLNESS_CHOICES"

interface Props {
  hasTimezone: boolean
}

const WellnessChoicesPage: NextPageWithLayout<Props> = ({ hasTimezone }) => {
    ...
    <>
      ...
      <section className="px-6 py-8 lg:px-8">
        <div className="mx-auto max-w-3xl">
          ...
          <ScenarioGamePlayer
            gameKey={GAME_KEY}
            correctLabel="Good call -- that's the healthier pick."
            incorrectLabel="Here's the choice that tends to serve you better:"
          />
        </div>
      </section>
    </>
...
WellnessChoicesPage.getLayout = (page) => <SiteLayout>{page}</SiteLayout>

export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  const session = await getServerSession(ctx.req, ctx.res, authOptions)
  if (!session) return { redirect: { destination: "/signin?callbackUrl=/tune-your-brain/wellness-choices", permanent: false } }

  const user = await db.user.findUnique({ where: { id: session.user.id } })
  return { props: { hasTimezone: !!user?.timezone } }
}

export default WellnessChoicesPage
