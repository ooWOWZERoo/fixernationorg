...
const GAME_KEY = "STRENGTH_SPOTTER"

interface Props {
  hasTimezone: boolean
}

const StrengthSpotterPage: NextPageWithLayout<Props> = ({ hasTimezone }) => {
    ...
    <>
      ...
      <section className="px-6 py-8 lg:px-8">
        <div className="mx-auto max-w-3xl">
          ...
          <ScenarioGamePlayer
            gameKey={GAME_KEY}
            correctLabel="Good eye -- that's exactly the strength at work."
            incorrectLabel="Take another look -- here's the strength that fits best:"
          />
        </div>
      </section>
    </>
...
StrengthSpotterPage.getLayout = (page) => <SiteLayout>{page}</SiteLayout>

export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  const session = await getServerSession(ctx.req, ctx.res, authOptions)
  if (!session) return { redirect: { destination: "/signin?callbackUrl=/tune-your-brain/strength-spotter", permanent: false } }

  const user = await db.user.findUnique({ where: { id: session.user.id } })
  return { props: { hasTimezone: !!user?.timezone } }
}

export default StrengthSpotterPage
