...
import { FreeTextGamePlayer } from "@/components/tuneBrain/FreeTextGamePlayer"
import { TimezoneCapture } from "@/components/tuneBrain/TimezoneCapture"

const GAME_KEY = "GRATITUDE_QUEST"

interface Props {
  hasTimezone: boolean
}

const GratitudeQuestPage: NextPageWithLayout<Props> = ({ hasTimezone }) => {
    ...
    <>
      ...
      <section className="px-6 py-8 lg:px-8">
        <div className="mx-auto max-w-3xl">
          ...
          <FreeTextGamePlayer gameKey={GAME_KEY} placeholder="What's something good you noticed today?" />
        </div>
      </section>
    </>
  )
...
GratitudeQuestPage.getLayout = (page) => <SiteLayout>{page}</SiteLayout>

export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  const session = await getServerSession(ctx.req, ctx.res, authOptions)
  if (!session) return { redirect: { destination: "/signin?callbackUrl=/tune-your-brain/gratitude-quest", permanent: false } }

  const user = await db.user.findUnique({ where: { id: session.user.id } })
  return { props: { hasTimezone: !!user?.timezone } }
}

export default GratitudeQuestPage
