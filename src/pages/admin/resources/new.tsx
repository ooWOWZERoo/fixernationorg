...
const AdminResourceNew: NextPageWithLayout = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await router.push(`/admin/resources/${data.id}`);
    } catch {
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div>
          ...
          <textarea
            ...
            rows={12}
            required
            placeholder="Full content. Line breaks carry through."
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-navy focus:outline-none focus:ring-1 focus:ring-navy"
          />
          <p className="mt-1 text-xs text-slate-400">Direct link to a PDF, worksheet, or other downloadable file.</p>
        </div>
...
AdminResourceNew.getLayout = (page) => <AdminLayout>{page}</AdminLayout>;
...
export default AdminResourceNew;
