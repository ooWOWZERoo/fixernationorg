...
const DRAFT_KEY = "fn_affiliate_draft";
const TOTAL_STEPS = 4;
...
const EMPTY: FormData = {
  ...
  howHeardAboutFN: "", audienceSize: "", platformsUsed: [], geographicFocus: "", whyJoining: "",
...
const BecomeAnAffiliatePage: NextPageWithLayout<Props> = ({ prefillEmail, prefillName }) => {
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 3) {
      ...
      if (!form.agreedToPolicy) { setStepError("Please agree to the affiliate program terms."); return false; }
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/applications/affiliate", {
        method: "POST",
      ...
      router.push(`/apply/confirmed?type=affiliate&email=${encodeURIComponent(form.email)}`);
    } catch {
  ...
  const steps = [
    ...
    <div key="s1" className="space-y-5">
      ...
      {fieldEl("Referral code", false, inputEl("referralCode", { placeholder: "Leave blank if none" }), "If someone already in the program sent you here, drop their code in.")}
    </div>,

    // Step 2 — How you'll promote
    <div key="s2" className="space-y-5">
      <div>
        <label className="mb-2 block text-sm font-semibold text-ink">Where will you share your link? <span className="font-normal text-ink-soft">(pick all that apply)</span></label>
      ...
      {fieldEl("Audience size", false, selectEl("audienceSize", AUDIENCE_SIZES, "Roughly how many people can you reach?"))}
      {fieldEl("Where are your people?", false, selectEl("geographicFocus", GEO_FOCUS),
        "If you'd like a territory tied to your name, tell us the area in the next question."
      )}
      {fieldEl("Why do you want to be an affiliate?", false,
        textareaEl("whyJoining", "Tell us who you'd be sharing Fixer Nation with, and mention any territory you have in mind.", 5)
      )}
    </div>,

    // Step 3 — Online presence
    <div key="s3" className="space-y-5">
      <p className="text-sm text-ink-soft">Every link here is optional. Share whichever ones show what you do.</p>
    ...
    <div key="s4" className="space-y-6">
      <div className="rounded-xl border border-navy/10 bg-cream-panel p-5 text-sm">
        ...
        <dl className="space-y-2">
          ...
          {form.platformsUsed.length > 0 && <div className="flex gap-2"><dt className="w-28 shrink-0 font-semibold text-ink">Channels</dt><dd className="text-ink-soft">{form.platformsUsed.join(", ")}</dd></div>}
        </dl>
        <button
          type="button"
      ...
      <div className="space-y-4">
        ...
        {[
          ...
          { field: "agreedToPolicy" as const, label: "I agree to Fixer Nation's community guidelines and affiliate program terms." },
      ...
      {fieldEl("Signature", true,
        ...
        "Typing your name here counts as your signature on this application."
      )}
    </div>,
  ];
  ...
  return (
    <>
      <Head>
        <title>Become an Affiliate — Fixer Nation</title>
        <meta name="description" content="Share Fixer Nation with your network and earn a commission on everyone who joins through your link. You don't need a business or a service listing." />
      </Head>
      ...
      <section className="bg-navy px-6 py-14 text-center lg:px-8">
        <div className="mx-auto max-w-xl">
          <h2 className="text-2xl font-extrabold text-white">Looking for something bigger?</h2>
          <p className="mt-3 text-sm text-white/75">Ambassadors represent Fixer Nation in their own community and hold a territory. Providers list the services they actually sell. Both come with affiliate commissions built in.</p>
          <div className="mt-6 flex flex-wrap justify-center gap-4">
            <Link href="/become-an-ambassador" className="inline-flex items-center justify-center rounded-[10px] bg-amber px-7 py-3 text-sm font-bold text-navy-dark no-underline shadow-[0_12px_24px_-10px_rgba(242,169,60,0.65)] transition-all hover:-translate-y-0.5 hover:bg-amber-dark">
              Become an ambassador
            </Link>
            <Link href="/become-a-provider" className="inline-flex items-center justify-center rounded-[10px] border-2 border-white/30 px-7 py-3 text-sm font-bold text-white no-underline transition-all hover:border-white/60 hover:bg-white/10">
              Become a provider
            </Link>
          </div>
        </div>
...
BecomeAnAffiliatePage.getLayout = (page) => <SiteLayout>{page}</SiteLayout>;
