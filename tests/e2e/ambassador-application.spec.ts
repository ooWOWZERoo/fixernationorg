...
const EMAIL = `qa-ambassador-app-${STAMP}@example.com`;
const FIRST_NAME = "QA";
const LAST_NAME = `Applicant${STAMP}`;

test.describe.configure({ mode: "serial" });

test("submit the ambassador application -> confirmation page -> appears in admin queue", async ({ page }) => {
  ...
  // Steps 2-5 (Background, Community, Why ambassador, Online presence) are
  // all optional — confirm Continue advances through each unfilled.
  ...
  await expect(row.getByText("AMBASSADOR").first()).toBeVisible();
});
